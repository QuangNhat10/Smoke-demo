...
namespace BreathingFree.Services
{
    public class AuthService
    {
        ...
        public async Task<bool> UpdateUserProfileAsync(int userId, ProfileUpdateModel model)
        {
            ...
            // Cập nhật các trường đặc biệt dành cho bác sĩ
            if (user.RoleID == 1) // Chỉ cập nhật các trường bác sĩ nếu user là bác sĩ
            {
                user.Specialty = model.Specialty;
                user.Degrees = model.Degrees;
                user.Workplace = model.Workplace;
                user.TreatmentMethods = model.TreatmentMethods;
                user.SuccessRate = model.SuccessRate;
                user.PatientCount = model.PatientCount;
                user.ResearchPublications = model.ResearchPublications;
                user.ResponseTimes = model.ResponseTimes;
