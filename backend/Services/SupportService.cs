...
public class SupportService
{
    ...
    {
        // Truy vấn tất cả tin nhắn giữa bác sĩ và các bệnh nhân, lấy thời gian cuối
        var lastTimes = await _context.Messages
            .Where(m => m.FromUserID == doctorId || m.ToUserID == doctorId)
            .Select(m => new
            {
                PatientId = m.FromUserID == doctorId ? m.ToUserID : m.FromUserID,
                m.SentAt
            })
        ...
        // lấy tất cả bệnh nhân (không phải bác sĩ) rồi lọc trên bộ nhớ
        var users = await _context.Users
            .Where(u => u.RoleID != 1)
            .ToListAsync();

        var result = users
            .Where(u => lastTimes.Any(l => l.PatientId == u.UserID))
            .Select(u => new SimpleUserDto
            {
                UserId = u.UserID,
                Name   = u.FullName,
                Avatar = u.Avatar,
                LastMessageTime = lastTimes
                    .First(l => l.PatientId == u.UserID)
                    .LastTime
            })
