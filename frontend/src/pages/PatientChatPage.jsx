...
const PatientChatPage = () => {
    ...
    useEffect(() => {
        if (!myUserId || !localStorage.getItem('token')) return;
        ...
        connection.start()
            .then(() => {
                console.log('SignalR connected (PatientChatPage)');
                connection.on('ReceiveMessage', (fromUserId, message) => {
                    console.log('ReceiveMessage (PatientChatPage):', { fromUserId, message });
                    setMessages(prev => [
                        ...prev,
                        {
                            chatID: Date.now(),
            ...
            .catch(err => console.error('SignalR connection error (PatientChatPage):', err));
    ...
    const handleScroll = () => {
        ...
        if (!el) return;
        const threshold = 120; // px distance from bottom considered "at bottom"
        const atBottom = el.scrollHeight - el.scrollTop - el.clientHeight < threshold;
        setIsAtBottom(atBottom);
    };

    // Auto-scroll to latest message only if user is already at bottom
    useEffect(() => {
    ...
    // Sort patients by lastMessageTime returned from backend (descending)
