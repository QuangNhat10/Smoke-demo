...
const Header = ({ userName }) => {
  ...
  useEffect(() => {
    ...
    // Ưu tiên lấy profilePicture, sau đó avatar, fallback ảnh mặc định
    const storedProfilePicture =
      localStorage.getItem("profilePicture") || localStorage.getItem("avatar");
    ...
    if (loggedIn && userId) {
      ...
      if (storedEmail) {
        ...
        setUserEmail(
          role === "Doctor" ? "doctor@example.com" : "user@example.com"
        );
      }
    }
  ...
  const handleProfileClick = () => {
    ...
    // Đảm bảo userId có giá trị trước khi chuyển hướng
    if (!localStorage.getItem("userId")) {
      // Nếu không có userId, người dùng cần đăng nhập lại
      console.warn("No userId found, user may need to login again");
    }
